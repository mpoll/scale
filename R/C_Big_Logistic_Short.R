...
############################################
    ...
    ### Specification of ScaLE
        ...
        p.num       <- 2^9      # Number of particles
        t.inc       <- 0.1
        T.extend    <- 1
        ss.size     <- 1

        base.vars <- c(2,2,2,2,2)
        x.init <- matrix(0,dimen,p.num); for(j in 1:dimen){x.init[j,] <- rnorm(p.num,beta.star[j]+1/(2*sqrt(dsz)),base.vars[j]*dsz^(-1/2))}
